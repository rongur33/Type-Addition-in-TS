...
import {
    ...
    BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType, makeTypePredTExp, ProcTExp, makeAnyTExp, TypePredTExp
} from "./TExp";
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<TExp> => {
    if (!isAppExp(e)) {
        return makeFailure("Not TypePred app");
    }

    if (isPrimOp(e.rator)) {
        if (e.rator.op === "string?") {
            return makeOk(makeTypePredTExp(makeStrTExp()));
        } else if (e.rator.op === "number?") {
            return makeOk(makeTypePredTExp(makeNumTExp()));
        } else if (e.rator.op === "boolean?") {
            return makeOk(makeTypePredTExp(makeBoolTExp()));
        } else {
            return makeFailure("not a predicate primOp");
        }
    }
    if (isVarRef(e.rator)) {
        const varTypeResult = <Result<ProcTExp>>applyTEnv(tenv, e.rator.var);
        if (varTypeResult.tag === "Ok") {
            const varType = varTypeResult.value;
            const compatibilityCheck = checkCompatibleType(varType.returnTE, makeTypePredTExp(makeAnyTExp()), e);
            if (compatibilityCheck.tag === "Ok") {
                return makeOk(varType.returnTE);
            } else {
                return makeFailure(compatibilityCheck.message);
            }
        } else {
            return makeFailure(varTypeResult.message);
        }
    }

    return makeFailure("not TypePred");
};

export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    const testTypeResult = isTypePredApp(ifExp.test, tenv);
    if (testTypeResult.tag === "Ok") {
        const testType = testTypeResult.value;
        return typeOfIfPredApp(testType as TypePredTExp, ifExp, tenv);
    } else {
...
// const isTypePredApp = (e: Exp, tenv: TEnv): Result<TExp> =>
//     !(isAppExp(e)) ? makeFailure("Not TypePred app") :
//         isPrimOp(e.rator) ?
//             e.rator.op === "string?" ? makeOk(makeTypePredTExp(makeStrTExp())) :
//                 e.rator.op === "number?" ? makeOk(makeTypePredTExp(makeNumTExp())) :
//                     e.rator.op === "boolean?" ? makeOk(makeTypePredTExp(makeBoolTExp())) : makeFailure("not a predicate primOp") :
//             isVarRef(e.rator) ?
//                 bind(<Result<ProcTExp>>applyTEnv(tenv, e.rator.var), (vType: ProcTExp) => {
//                     return either(
//                         checkCompatibleType(vType.returnTE, makeTypePredTExp(makeAnyTExp()), e),
//                         (compatible: true) => makeOk(vType.returnTE),
//                         (fail: string) => makeFailure(fail)
//                     );
//                 }) :
//                 makeFailure("not TypePred");
//
//
//
// export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
//     either(
//         bind (isTypePredApp(ifExp.test, tenv), (t:TExp) => typeOfIfPredApp(<TypePredTExp>t, ifExp, tenv)),
//         makeOk,
//         () => typeofIfNormal(ifExp, tenv));

export const typeOfIfPredApp = (predType:TypePredTExp, ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    const testTE = typeofExp(ifExp.test, tenv);
    const constraint1 = bind(testTE, testTE => checkCompatibleType(testTE, makeTypePredTExp(makeAnyTExp()), ifExp));
    return bind(constraint1, (_c1: true) =>
        makeOk(predType.paramTE));
};
