...
export type NonTupleTExp = AtomicTExp | ProcTExp | TVar | UnionTExp|InterTExp|TypePredTExp;
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTVar(x) || isUnionTExp(x)||isTypePredTExp(x);
...
export type TypePredTExp = { tag: "TypePredTExp"; paramTE: TExp;  };
export const makeTypePredTExp = (paramTE: TExp): TypePredTExp =>({tag: "TypePredTExp", paramTE: paramTE});
...
// Function to unparse and compare TExps
...
//     }
//
//     // Case 1: te1 is UnionTExp, te2 is UnionTExp
//     if (isUnionTExp(te2)) {
//          return makeInterTExp( te2.components.map(component => makeDiffTExp(te1,component)));
//     }
//     // Case 2: te1 is InterTExp, te2 is InterTExp
//     if (isInterTExp(te2)) {
//         return makeUnionTExp( te2.components.map(component => makeDiffTExp(te1,component)));
//     }
//     // Case 3: te1 is UnionTExp, te2 is InterTExp
//     if (isUnionTExp(te1)) {
//         return makeUnionTExp( te1.components.map(component => makeDiffTExp(component,te2)));
//     }
//     // Case 4: te1 is InterTExp, te2 is UnionTExp
//     if (isInterTExp(te1)) {
//         return makeInterTExp( te1.components.map(component => makeDiffTExp(component,te2)));
//     }
//     // Edge case: if none of the above conditions are met
//     return makeNeverTExp();
// };
...
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp => {
    ...
    const diffComponents = (components1: TExp[], components2: TExp[]): TExp[] =>
        differenceWith(equals, components1, components2);

    // Case 1: te1 is UnionTExp, te2 is UnionTExp
    if (isUnionTExp(te1) && isUnionTExp(te2)) {
        const diff = diffComponents(te1.components, te2.components);
        return diff.length === 1 ? diff[0] : makeUnionTExp(diff);
    }

    // Case 2: te1 is InterTExp, te2 is InterTExp
    if (isInterTExp(te1) && isInterTExp(te2)) {
        const diff = diffComponents(te1.components, te2.components);
        return diff.length === 1 ? diff[0] : makeInterTExp(diff);
    }

    // Case 3: te1 is UnionTExp, te2 is InterTExp
    if (isUnionTExp(te1) && isInterTExp(te2)) {
        const diff = te2.components.map(component => makeDiffTExp(te1,component));
        return makeUnionTExp(diff);
    }

    // Case 4: te1 is InterTExp, te2 is UnionTExp
    if (isInterTExp(te1) && isUnionTExp(te2)) {
        const diff = te1.components.map(component => makeDiffTExp(component, te2));
        return makeInterTExp(diff);
    }

    // Case 5: te1 is AtomicTExp, te2 is UnionTExp
    if (isAtomicTExp(te1) && isUnionTExp(te2)) {
        return contains(te1, te2.components) ? makeNeverTExp() : te1;
    }

    // Case 6: te1 is AtomicTExp, te2 is InterTExp
    if (isAtomicTExp(te1) && isInterTExp(te2)) {
        return contains(te1, te2.components) ? makeNeverTExp() : te1;
    }

    // Case 7: te1 is UnionTExp, te2 is AtomicTExp
    if (isUnionTExp(te1) && isAtomicTExp(te2)) {
        const diff = te1.components.filter(component => !equals(component, te2));
        return diff.length === 1 ? diff[0] : makeUnionTExp(diff);
    }

    // Case 8: te1 is InterTExp, te2 is AtomicTExp
    if (isInterTExp(te1) && isAtomicTExp(te2)) {
        const diff = te1.components.filter(component => !equals(component, te2));
        return diff.length === 1 ? diff[0] : makeInterTExp(diff);
    }
...
export const isSubType = (te1: TExp, te2: TExp): boolean => {
    ...
    // Check if te1 is contained within the components of te2
    if (isInterTExp(te2)) {
    ...
    // Default case: return false if none of the conditions are met
    return false;
};
...
//             (isInterTExp(te1) && isUnionTExp(te2)) ? isSubset(te1.components, te2.components) :
//                 (isUnionTExp(te1) && isInterTExp(te2)) ? isSubset(te1.components, te2.components) :
        ...
        (texp === "boolean") ? makeOk(makeBoolTExp()) :
            (texp === "void") ? makeOk(makeVoidTExp()) :
                (texp === "string") ? makeOk(makeStrTExp()) :
                 ...
                 //           (texp==="is?")?parseTypePredTExp(texp):
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> =>
    (texps[0] === "union") ? parseUnionTExp(texps) :
        (texps[0]==="inter")?parseInterTExp(texps):
            texps.indexOf('is?') !== -1 ? parseTypePredTExp(texps) :
        parseProcTExp(texps);
...
// const parseTypePredicate=(texps: Sexp[]):Result<ProcTExp> {
//
// }
...
//     bind(parseTExp(texps.slice(1)[0]), (pred:TExp) => makeOk(makeTypePredTExp(pred)));
const parseTypePredTExp = (texps: Sexp[]): Result<TypePredTExp> => {
    const parsedTExpResult = parseTExp(texps[1]);

    if (parsedTExpResult.tag === "Ok") {
        const pred = parsedTExpResult.value;
        return makeOk(makeTypePredTExp(pred));
    } else {
        return makeFailure(parsedTExpResult.message);
    }
};
/*
                                    ...
                                    isTVar(x) ? up(tvarContents(x)) :
                                        isUnionTExp(x) ? mapv(mapResult(unparseTExp, x.components), (componentTEs: string[]) =>
                                            ...
                                            isInterTExp(x)?mapv(mapResult(unparseTExp, x.components), (componentTEs: string[]) =>
                                                ...
                                                isTypePredTExp(x) ? bind(unparseTExp(x.paramTE), (paramTE: string) =>
                                                        makeOk(['is?', paramTE])) :
